...
module.exports = function() {
  ...
  io.sockets.on('connection', (socket) => {
    ...
    socket.on('UPDATE_ROOM_SETTINGS', ({roomId, updatedSettings}) => {
      socketMetricInc('updateRoomSettings');
      // controllers.rooms
        // .getById(roomId)
        // .then((res) => {
        //   const { settings } = res;
          socket.broadcast
            .to(roomId)
            .emit('RECEIVED_UPDATED_ROOM_SETTINGS', updatedSettings)

          const message = new Message({
            room: roomId,
            settings,
            text: `Room Settings Changed: ${updatedSettings}`,
            messageType: 'UPDATED_ROOM_SETTINGS',
            autogenerated: true,
            timestamp: new Date().getTime(),
          })
