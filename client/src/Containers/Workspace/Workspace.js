...
class Workspace extends Component {
  ...
  componentDidUpdate(prevProps) {
    const { user, temp, lastMessage, controlledBy } = this.props;
    const { currentTabId } = this.state;

    if (!socket.connected && controlledBy[currentTabId] === user._id) {
      const auto = true;
  ...
  initializeListeners = () => {
    ...
    socket.on('TOOK_CONTROL', (message) => {
      ...
      this.addToLog(message);
      setControlledBy(message.tab, message.user._id);
      this.setState({
  ...
  toggleControl = (event, auto) => {
    ...
    if (controlledBy[currentTabId] === user._id) {
  ...
  render() {
    ...
    const currentMembers = (
      <CurrentMembers
        ...
        activeMember={controlledBy[currentTabId]}
...
Workspace.propTypes = {
  ...
  controlledBy: PropTypes.shape({}).isRequired,
